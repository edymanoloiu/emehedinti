...
export async function getServerSideProps({ params, query }) {
	...
	const canonicalLocalSlug = getLocalCategorySlug();

	// Collapse legacy local-category URLs onto the canonical slug.
	if (isLocalCategorySlug(postParams) && postParams !== canonicalLocalSlug) {
		return {
			redirect: {
				destination:
					page > 1
						? `/categorie/${canonicalLocalSlug}/?page=${page}`
						: `/categorie/${canonicalLocalSlug}/`,
				permanent: true,
			},
		};
	}
	...
	const getCategoryData = allPosts
		.filter((post) => !isRecomandarePost(post) && postMatchesCategorySlug(post.cate, postParams))
	...
	const categoryName = isLocalCategorySlug(postParams)
		? (publication.localCate || getCategoryData[0].cate)
		: getCategoryData[0].cate;
	const categorySlug = isLocalCategorySlug(postParams) ? canonicalLocalSlug : postParams;
