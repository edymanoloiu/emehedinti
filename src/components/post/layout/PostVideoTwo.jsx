		...
		<div className={`media post-block post-block__small ${pClass ?? "post-block__on-dark-bg m-b-xs-30"}`}>
			...
			<div className="media-body">
				<div className="post-cat-group">
					<Link href={`/categorie/${slugify(data.cate)}`} className={`post-cat ${data.cate_bg ?? "bg-color-blue-one"}`}>{data.cate}</Link>
				</div>
				...
				<div className="post-metas">
					<ul className="list-inline">
						<li>
							<span>De la</span>
							<Link href={`/autor/${slugify(data.author_name)}`} className="post-author">{data.author_name}</Link>
						</li>
					</ul>
				</div>
			</div>
