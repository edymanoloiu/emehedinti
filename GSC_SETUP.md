# Google Search Console setup — emehedinti.ro
...
1. Verify ownership of `https://emehedinti.ro` in [Google Search Console](https://search.google.com/search-console).
2. Prefer the URL-prefix property `https://emehedinti.ro/`.
...
2. Create a JSON key and store it **outside** the repo (e.g. `~/.config/gsc/emehedinti.json`).
...
```bash
export GOOGLE_APPLICATION_CREDENTIALS="$HOME/.config/gsc/emehedinti.json"
export GSC_SITE_URL="https://emehedinti.ro/"
export GSC_SITEMAP_URL="https://emehedinti.ro/sitemap-index.xml"
```
...
1. Confirm `https://emehedinti.ro/sitemap-index.xml` returns 200.
